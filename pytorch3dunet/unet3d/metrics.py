...
from pytorch3dunet.augment.transforms import CropCube
import math
import time
...
class CustomIoU:
    """
    Computes IoU for each class separately and then averages over all classes. Supports slice-based (2D) and 3D evaluations on 3D volumes
    """

    def __init__(self, skip_channels=(), ignore_index=None, perfect_empty = 'one', z_only = False, include_empty=True, **kwargs):
        """
        :param skip_channels: list/tuple of channels to be ignored from the IoU computation
        :param ignore_index: id of the label to be ignored from IoU computation
        """
        self.z_only = z_only
        self.perfect_empty = perfect_empty
        self.ignore_index = ignore_index
        self.skip_channels = skip_channels
        self.include_empty = include_empty

    def __call__(self, input, target, binarize_input=True, binarize_target=False):
        """
        :param input: 5D probability maps torch float tensor (NxCxDxHxW)
        :param target: 4D or 5D ground truth torch tensor. 4D (xDxHNxW) tensor will be expanded to 5D as one-hot
        :return: intersection over union averaged over all channels
        """
        # assert input.dim() == 5
        ...
        for _input, _target in zip(input, target):
            ...
            binary_prediction = self._binarize_predictions(_input, n_classes) if binarize_input else _input
            _target = self._binarize_predictions(_target, n_classes) if binarize_target else _target
            ...
            for c in range(n_classes):
                ...
                if self.z_only == True:
                    ...
                    per_channel_iou.append(self._jaccard_index_calculated_along_z(binary_prediction[c], _target[c]))
    ...
    def _jaccard_index(self, prediction, target):
        ...
        Computes IoU for a given target and prediction tensors
        """
        bot = torch.sum(prediction | target).float()
        if bot == 0 and self.perfect_empty != "zero":
            if self.perfect_empty == "nan":
                return torch.tensor(float('nan'))
            elif self.perfect_empty == "one":
                return torch.tensor(1.0)
        return torch.sum(prediction & target).float() / torch.clamp(bot, min=1e-8)
    
    def _jaccard_index_calculated_along_z(self, prediction, target):
        """
        Computes IoU for a given target and prediction tensors
        """
        iou_list = []
        for idx in range(prediction.shape[0]):
            bot = torch.sum(prediction[idx] | target[idx]).float()
            if bot == 0:
                if self.perfect_empty == "nan":
                    two_d_iou =  torch.tensor(float('nan'))
                elif self.perfect_empty == "one":
                    two_d_iou =  torch.tensor(1.0)
            else:
                two_d_iou = torch.sum(prediction[idx] & target[idx]).float() / torch.clamp(bot, min=1e-8)
            iou_list.append(two_d_iou)
        return torch.nanmean(torch.tensor(iou_list))

def IoU_2D(input, target, binarize_input=True, binarize_target=False, if_empty='nan'):
    ...
    input = input.byte()
    target = target.byte()
    bot = torch.sum(input | target).float()
    top = torch.sum(input & target).float()
    if bot == 0:
        if if_empty =="nan":
            two_d_iou = torch.tensor(float('nan')) 
        else: 
        ...
        two_d_iou = torch.sum(input & target).float() / torch.clamp(bot, min=1e-8)
