...
from pytorch3dunet.unet3d.utils import get_logger, get_tensorboard_formatter, create_sample_plotter, create_optimizer, \
    create_lr_scheduler, get_number_of_learnable_parameters, create_warmup_scheduler
from . import utils
from pytorch3dunet.unet3d.prob_utils import l2_regularisation
...
def _create_trainer(config, model, optimizer, lr_scheduler, warmup_scheduler, loss_criterion, eval_criterion, loaders):
    ...
    if resume is not None:
        ...
        return ProbUNet3DTrainer.from_checkpoint(model=model,
                                             optimizer=optimizer,
        ...
        return ProbUNet3DTrainer.from_pretrained(model=model,
                                             optimizer=optimizer,
                                             lr_scheduler=lr_scheduler,
                                             loss_criterion=loss_criterion,
                                             eval_criterion=eval_criterion,
        ...
        return ProbUNet3DTrainer(model=model,
                             optimizer=optimizer,
                             lr_scheduler=lr_scheduler,
...
class ProbUNet3DTrainerBuilder:
    @staticmethod
    def build(config):

        wandb.init(project="prob3dunet", entity="sudo-chris", config = config)
        ...
        # warm-up
        warmup_scheduler = None # create_warmup_scheduler(config.get('warmup', None), optimizer) 

        # Create model trainer
        trainer = _create_trainer(config, model=model, optimizer=optimizer, lr_scheduler=lr_scheduler, warmup_scheduler = warmup_scheduler,
                                  loss_criterion=loss_criterion, eval_criterion=eval_criterion, loaders=loaders)

        return trainer


class ProbUNet3DTrainer:
    """3D UNet trainer.
    ...
    Args:
        ...
        warmup_scheduler: learning rate warmup
        ...
        elbo_score_lower_is_better (bool): if True higher elbo scores are considered better
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion, 
                 eval_criterion, device, loaders, checkpoint_dir, warmup_scheduler= None,
        ...
        self.elbo_score_lower_is_better = elbo_score_lower_is_better

        logger.info(model)
        logger.info(f'elbo_score_lower_is_better: {elbo_score_lower_is_better}')


        # initialize the best_eval_score
        if elbo_score_lower_is_better:
            self.best_elbo_score = float('+inf')
        else:
            self.best_elbo_score = float('-inf')
    ...
    def from_checkpoint(cls, resume, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders,
        ...
        logger.info(
            f"Checkpoint loaded. Epoch: {state['epoch']}. Num_iterations: {state['num_iterations']}")
        ...
        return cls(model, optimizer, lr_scheduler,
                   ...
                   elbo_score_lower_is_better=state['elbo_score_lower_is_better'],
    ...
    def from_pretrained(cls, pre_trained, model, optimizer, lr_scheduler, loss_criterion, eval_criterion,
        ...
        return cls(model, optimizer, lr_scheduler,
                   ...
                   elbo_score_lower_is_better=elbo_score_lower_is_better,
    ...
    def train(self):
        ...
        for t in self.loaders['train']:
            ...
            if self.num_iterations % self.validate_after_iters == 0:
                ...
                is_best = self._is_lowest_elbo(eval_loss)
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if not self.skip_train_validation:
                    ...
                    # self._log_params()
                    self._log_images(input[:2], target[:2], output[:2], 'train_')


                    wandb.log({'train_loss': train_losses.avg, 'train_score':train_eval_scores.avg, 'lr': self.optimizer.param_groups[0]['lr']})
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, t in enumerate(self.loaders['val']):
                ...
                if hasattr(self.model, 'final_activation') and self.model.final_activation is not None:
                    output = self.model.final_activation(output)
                self._log_images(input[:2], target[:2], output[:2], 'val_')
            ...
            wandb.log({'val loss': val_losses.avg, 'val_score':val_scores.avg})
            return val_scores.avg, loss
    ...
    def _forward_pass(self, input, target, weight=None):
        ...
        # forward pass
        output, kl_div = self.model.forward(input, target)
        # """
        # Calculate the evidence lower bound of the log-likelihood of P(Y|X)
        # """
        wandb.log({'kl_div': kl_div.sum()})
        if weight is None:
            elbo = self.loss_criterion(output, target, kl_div.sum(), self.num_iterations)
        else:
            elbo = self.loss_criterion(output, target, weight, kl_div.sum(), self.num_iterations)

        # if isinstance(self.model, nn.DataParallel):
        #     reg_loss = l2_regularisation(self.model.module.posterior) + l2_regularisation(self.model.module.prior) + l2_regularisation(self.model.module.fcomb.layers)
        # else:
    ...
    def _is_lowest_elbo(self, elbo):

        is_best = elbo < self.best_elbo_score

        if is_best:
            logger.info(f'Saving new best evaluation metric: {elbo}')
            self.best_elbo_score = elbo
    ...
    def _save_checkpoint(self, is_best):
        ...
        utils.save_checkpoint({
            ...
            'elbo_score_lower_is_better': self.elbo_score_lower_is_better,
    ...
    def _log_images(self, input, target, prediction, prefix=''):
        ...
        for name, batch in img_sources.items():
            for tag, image in self.tensorboard_formatter(name, batch):
                ...
                if "predictions" in tag:
                    segmentation = wandb.Image(image>0.5)
                    wandb.log({f"{prefix}{tag}_segmentation": segmentation})
