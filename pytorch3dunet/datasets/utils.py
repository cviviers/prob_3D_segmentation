...
def get_validation_loaders(config):
    """
    Returns dictionary containing the training and validation loaders (torch.utils.data.DataLoader).
    :param config: a top level configuration object containing the 'loaders' key
    :return: dict {
    ...
    val_datasets = dataset_class.create_datasets(loaders_config, phase='complete_val')
    ...
    # when training with volumetric data use batch_size of 1 due to GPU memory constraints
    # return DataLoader(ConcatDataset(val_datasets), batch_size=batch_size, shuffle=False, num_workers=num_workers)
    # use generator in order to create data loaders lazily one by one
    for val_dataset in val_datasets:
        logger.info(f'Loading val set from: {val_dataset.file_path}...')
        if hasattr(val_dataset, 'prediction_collate'):
            collate_fn = val_dataset.prediction_collate
        else:
            collate_fn = default_prediction_collate

        yield DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
