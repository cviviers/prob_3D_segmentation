...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path,
        ...
        :param one_label (bool): if a list of labels are provided, randomly use one during training. 
        """
        assert phase in ['train', 'val', 'test', 'complete_val']
        ...
        if phase in ["train", "val", "complete_val"]:
        ...
        # # randomly choose a label to use
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'test':
        ...
        elif self.phase == 'complete_val':
            # get the slice for a given index 'idx'
            label_idx = self.label_slices[idx]
            if self.one_label: 
                selected_label = random.randint(0,len(self.labels)-1)
                label_patch_transformed = self._transform_patches([self.labels[selected_label]], label_idx, self.label_transform)
            else: 
            ...
            if self.weight_maps is not None:
                ...
                return raw_patch_transformed, label_patch_transformed, weight_patch_transformed, raw_idx

            # return the transformed raw and label patches
            return raw_patch_transformed, label_patch_transformed, raw_idx
        else:
            # get the slice for a given index 'idx'
            label_idx = self.label_slices[idx]
            if self.one_label: 
        ...
        for file_path in file_paths:
            try:
                ...
                dataset = cls(file_path=file_path,
                              ...
                              one_label = dataset_config.get('one_label', False))
...
class StandardHDF5Dataset(AbstractHDF5Dataset):
    ...
    def __init__(self, file_path, phase, slice_builder_config, transformer_config, mirror_padding=(16, 32, 32),
                 raw_internal_path='raw', label_internal_path='label', weight_internal_path=None, random_seed = 0,
                 one_label = False):
...
#                  instance_ratio=None, random_seed=0, one_label = False):
#         super().__init__(file_path=file_path,
#                          phase=phase,
#                          slice_builder_config=slice_builder_config,
#                          transformer_config=transformer_config,
...
class LazyHDF5Dataset(AbstractHDF5Dataset):
    ...
    def __init__(self, file_path, phase, slice_builder_config, transformer_config, mirror_padding=(16, 32, 32),
                 raw_internal_path='raw', label_internal_path='label', weight_internal_path=None, random_seed = 0, one_label = True):
        super().__init__(file_path=file_path,
                         phase=phase,
                         slice_builder_config=slice_builder_config,
                         transformer_config=transformer_config,
